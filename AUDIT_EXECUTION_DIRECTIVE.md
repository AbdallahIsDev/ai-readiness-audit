...
controls:
  ...
  FIXED_AUDIT_FEE_USD: 500        # SINGLE offer price: full written audit + priority fixes. No second tier (§12.4).
...
**Critical ordering rule — test the way a REAL AI agent retrieves the site, not the way a human does.** Real AI assistants (ChatGPT, Gemini, Perplexity, Google's AI) answer by making a **web fetch / search-tool request** — they do NOT open a browser and click. So the MOST important test is the fetch simulation (check 2), which must run FIRST and before any browser work. If key business information (pricing, hours, services, contact) is invisible to a plain fetch but visible to a browser, that is a HIGH-VALUE finding — the AI agent cannot help this business even though a human can. The browser test (check 3) is a secondary, different lens — it simulates a human's AI assistant using agentic browsing — and runs after the fetch test. Never let the browser test substitute for the fetch test.

1. **Technical Accessibility Scan** (deterministic) — this is a WIDE scan producing as many REAL findings as the site genuinely warrants, across: `llms.txt`; `robots.txt` rules affecting AI crawlers (GPTBot, ClaudeBot/Claude-User, PerplexityBot, Google-Extended, etc.); schema.org/JSON-LD structured data (LocalBusiness + type-specific markup); sitemap.xml; whether key content (pricing, hours, services/menu, location/contact) is real extractable text vs locked in images or JS-only rendering; HTTPS + security headers; **tech-stack identification (§6.2)**; basic SEO/accessibility signals (unique titles, meta descriptions, alt text, heading hierarchy, mobile rendering). The goal is breadth: find EVERY genuine, verifiable problem, not just the llms.txt/robots pair. Never invent findings — but never stop at the first few either. Do not inflate counts with trivial noise; do not omit real, verifiable problems.
2. **AI-Fetch Simulation — what a real AI tool receives** (behavioral, HIGHEST priority, runs FIRST) — simulate exactly what an AI assistant's web-fetch/search tool gets: fetch the homepage and key pages with a plain HTTP GET (no browser, no JavaScript execution, no rendering). Attempt to extract pricing, hours, services/menu, location, contact, booking options from what the raw fetch returns. This is what a real AI agent literally sees. Record exactly what it could and couldn't get, and where the raw response is empty, JS-shell-only, or missing critical business facts. **This is the primary behavioral test** — findings here are the most defensible to a client because they reproduce exactly what happens when a customer asks an AI about their business.
...
**Tier 3 — Live AI assistant test (ChatGPT + Gemini).** Using Browser Use, navigate to the web interfaces of ChatGPT and Gemini (no authentication needed — the free/landing pages accept prompts without login). For each, enter a prompt like: "Fetch everything you can about [domain] — tell me their services, prices, hours, location, contact info, and how to book." Wait for the response. **Screenshot the full AI reply** — this is the PRIMARY proof image for the entire audit. Then compare the AI's answer against the known-complete baseline from Tier 2. Any information the AI assistant missed or got wrong = a real finding, documented with the screenshot as Gate-1 evidence. **The screenshot of the AI's reply is the single most persuasive piece of evidence in the whole workflow**: a client-facing screenshot showing ChatGPT or Gemini giving incomplete/wrong information about their own business. Save it as `Screenshots/<domain>_aiassistant_chatgpt.png` and `Screenshots/<domain>_aiassistant_gemini.png` — these are the images used in the outreach email (§12.4) and the client report.

**Tier 3b — AI research of the business economics (feeds the left-on-table figure).** In the SAME session with each assistant, ask a follow-up economics question that instructs it to SEARCH the web (not answer from memory): "Do web research and tell me: for a [industry type] business like [domain], what is the average revenue per client/job, and what does a typical such business earn per day, per week, and per month? Search the web for real figures." **Screenshot both economics answers** (`Screenshots/<domain>_aiassistant_chatgpt_economics.png` / `_gemini_economics.png`). These researched figures are the source for the left-on-table estimate in §12.6 — the agent must NOT invent its own numbers. If the assistants disagree wildly or can't research, the estimate is omitted per §12.6.
...
Every site's README must state the technology the website is built with — the client needs to know how a fix would be delivered (custom code vs no-code platform), and you need it to scope the done-for-you tier.

**Detect by inspecting:** the HTML source (`<meta name="generator">`, build markers like `__NEXT_DATA__` (Next.js), `_next/` assets (Next.js), `wp-content` (WordPress), `cdn.shopify.com` (Shopify), `wix.com`/`static1.squarespace.com` (Wix/Squarespace), Webflow assets (`assets.website-files.com`), Framer, Vue `__NUXT__`, Astro, Gatsby, Hugo, static HTML, a known CMS, or a custom build), the response headers (`X-Generator`, `Server`, `x-powered-by`), and what the page structure implies (a site builder's recognizable blocks, an e-commerce platform's checkout URL patterns). Combine 2+ signals before concluding.
...
- **Tech Stack:** <detected platform/framework per §6.2 — e.g. "WordPress (with Elementor)", "Next.js (custom build)", "Shopify", "Webflow", "Static HTML", "Unknown — appears custom-built">
...
3. **Proof (1–2 sentences + screenshot):** the email must convince a skeptical owner this is real, not a scam. State the count of concrete problems found on THEIR site ("I found 8 specific things on your site that do this"). Then, if a defensible estimate exists per §12.6, state the approximate left-on-table figure: "Conservatively, this likely costs you around $X/year in lost jobs." **The single strongest proof is a screenshot of ChatGPT or Gemini giving incomplete/wrong information about their own business** — captured live during Tier 3 of the audit (§6.1). Embed it directly: `![ChatGPT's answer about YOUR business](Screenshots/<domain>_aiassistant_chatgpt.png)`. A screenshot of an ABSENT thing (like a missing contact form) is useless — only screenshot what exists. If the AI assistant test failed (auth wall, captcha), fall back to a screenshot of a visible problem on their site.
4. **Offer (1 sentence, ONE option only):** a single, clear offer — a full written audit with the priority fixes done, for `$FIXED_AUDIT_FEE_USD`. No second/cheaper tier, no "report-only" option, no dashes between the offer and the price, no pricing gymnastics. One price, one thing you do. Example: "I'll send you the full written audit with the priority fixes in place, all for $500."
5. **CTA (1 sentence):** a single, concrete, low-pressure ask that does NOT give the work away for free. BANNED: "Reply 'send it' and I'll email you the summary", "I'll email the full list, just reply", or any wording implying the full findings/fixes ship before payment — the owner would take the list and fix it themselves, making you redundant. Use a direct next step toward the paid offer, e.g. "If you'd like, reply and I'll get the audit started this week." or "Reply 'let's do it' and I'll begin." No "comment 'X'" patterns, no gimmicky engagement bait.
...
### 12.6 Business intelligence — the left-on-table estimate (sourced from AI research, NEVER guessed)

Before writing the email, the copywriter must understand THIS business's money with REAL numbers — not an invented figure, not a guessed annual total. The number must be large enough to be real (a trade business can lose thousands per single client, not "$3,000/year") and defensible.

**The research pass is MANDATORY and is done LIVE through ChatGPT AND Gemini — never from the agent's own judgment.**

1. **Ask both AI assistants to research the industry economics.** During the Tier 3 live test (§6.1), after the site-information question, ask each assistant (in the same session) a follow-up economics question, explicitly instructing it to SEARCH (web research), not answer from training memory:
   - "Do web research and tell me: for a [industry type] business like [domain], what is the average revenue per client/job, and what does a typical such business earn per day, per week, and per month? Search the web for real figures."
   - Use the business type you identified (electrician, salon, plumber, etc.) — NOT a generic "small business" question.
2. **Capture the replies.** Screenshot both assistants' economics answers (`Screenshots/<domain>_aiassistant_chatgpt_economics.png` / `_gemini_economics.png`) and record the figures in the worklog with the source.
3. **Build the left-on-table number from the researched figures.** `(average revenue per client from AI research) × (conservative count of clients lost to the audited problems)`. The result is the money-left-on-table figure. It should be stated per a realistic period — per week, per month, or per year — whichever is most defensible. Do NOT present a small annual number for a high-ticket industry; if the researched per-client figure is thousands, the loss figure must reflect that scale.
4. **The number appears only if both assistants give consistent, researched figures.** If ChatGPT and Gemini disagree wildly, or either refuses/can't research, omit the dollar figure entirely rather than fabricate it.

**Where it goes:** the left-on-table number appears in the Outreach body (§12.4 point 3) and MAY appear once in the audit Executive Summary. **The reasoning and source must be visible, not just the number** — e.g. "based on industry research, an [industry] job averages ~$X; losing just one of those a month is ~$X × 12 ≈ $X/year." Attribute the figure: "according to web research via ChatGPT/Gemini, the average [industry] job in [region] is ~$X." A number with no visible reasoning and no source reads as fabricated.

**Truthfulness bar:** the estimate MUST trace to (a) the live ChatGPT/Gemini research answers with web search (screenshots on file), and (b) a conservative reasoning chain WRITTEN OUT in the deliverable. It must NEVER be a rounded-up guess, never a severity-weight formula, never a number invented to impress, never a small annual figure that contradicts the industry's real per-client revenue. If the research can't support a defensible figure with a visible chain and source, OMIT the number rather than fabricate it. Consistency: the SAME figure appears in the audit Executive Summary and the Outreach.
...
- If a left-on-table figure is included, it MUST be sourced from the §6.1 Tier 3b ChatGPT/Gemini web research (economics screenshots on file), with the reasoning and source written out inline, and the figure matches the audit's Executive Summary (§12.6); if not AI-researched, no figure was invented.
- CTA is the honest direct alternative — the banned "Reply 'send it'..." phrasing is never used, and near-variants like "just reply and I'll send it" are also avoided in favor of "Reply and I'll get started" (§12.4 point 5). The CTA does NOT imply the full findings/fixes ship before payment (§12.4 point 5).
- Pricing is a SINGLE option: full audit + priority fixes at `$FIXED_AUDIT_FEE_USD` — no second/cheaper tier, no "report-only" option, no dashes between the offer and the price (§12.4 point 4).
...
- **Screenshot discipline starts at capture time** (screenshots dominate the zip size — this is the actual lever):
  - Capture browser screenshots as **SECTION-SCOPED captures** (§4.6 — the relevant section/element, sized to it, never a full-page capture) at a maximum width of **1000px** (smaller where the page allows). Never full-resolution 1280px+ captures, never full-page captures.
