...
| Item | Purpose |
|---|---|
| `AUDIT_EXECUTION_DIRECTIVE.md` | The master directive (v3 dual-mode). One copy-paste prompt that runs an entire session in ONE `TARGET_MODE`: `with_website` = full 11-check audit (AI-readiness as ONE bucket, outdated-site/redesign PRIMARY, fetch-first → browser → ChatGPT/Gemini live test → free tools, evidence gates, audit→redesign outreach) OR `no_website` = Maps/social discovery → verify-no-website → dossier + build outreach, **plus an optional per-business design system + single-page landing page under the `GENERATE_LANDING_PAGE` toggle (§3C)**. **Both modes target high-income markets only (§3.5 — `TARGET_COUNTRIES`; Egypt is never a target, and neither is any other low / lower-middle / upper-middle-income economy)**, **both modes run the same §3.6 contact-hunt** (every email, every phone, every DM-able social profile; channel priority Email > Social DM > Phone-messaging; phones never called), and both produce the session-level **`OUTREACH_CONTACTS.md` click-to-contact sheet (§3.7)**, which the agent must read back and verify before continuing. Mode router (§0), geographic targeting (§3.5), contact-hunt + contact sheet (§3.6–§3.7), discovery workflow (§3B), landing-page generation (§3C), dossiers (§11/§11B), outreach (§12/§12B incl. §12.4R redesign close), ledger (§14/§14B), packaging (§15/§15B), done-gates (§16/§16B), reports (§17), hard limits (§18/§18B). |
...
- **TWO client-sensitive files exist**, both zip-only and both blocked by `.gitignore`: (a) each target's `websites/<site>/README.md` business dossier (emails, phones, owner names, addresses), blocked by the scoped rule `websites/*/README.md`; and (b) the session-level `OUTREACH_CONTACTS.md` contact sheet (directive §3.7), which aggregates every business's channels into one click-to-contact file, blocked by its own root rule. Neither ever reaches GitHub — both are delivered only inside `Deliverables.zip`.
...
```
1. Edit §0 Controls: TARGET_MODE (with_website | no_website) + quantity (NUMBER_OF_WEBSITES or NUMBER_OF_NO_WEBSITE_BUSINESSES) + TARGET_GROUP + **TARGET_COUNTRIES (the high-income geo allow-list — default `US, CA, GB, IE, AU, NZ, SG`; Egypt is never valid)** + fees + GENERATE_LANDING_PAGE (ON/OFF — no_website mode only; ON = the agent authors a design system per business, then builds the page from it — no external prerequisite).
